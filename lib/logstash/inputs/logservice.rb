...
class LogStash::Inputs::LogService < LogStash::Inputs::Base
  ...
  config :processor, :validate => :string, :default => nil
  ...
  def run(queue)
    ...
    @logHubStarter.startWorker(@endpoint, @access_id, @access_key, @project, @logstore, @consumer_group, @consumer_name + @ip_suffix + @process_pid, @position, @checkpoint_second, @include_meta, @blockingQueue, @proxy_host, @proxy_port, @proxy_username, @proxy_password, @proxy_domain, @proxy_workstation, @fetch_interval_millis, @processor)
    ...
    consume(queue)
        ...
        @logger.error("Start logstash-input-logservice", :endpoint => @endpoint, :project => @project, :logstore => @logstore,
            :consumer_group => @consumer_group, :consumer_name => @consumer_name, :position => @position,
            :checkpoint_second => @checkpoint_second, :include_meta => @include_meta, :consumer_name_with_ip => @consumer_name_with_ip,
            :processor => @processor, :exception => e)
